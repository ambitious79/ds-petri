...
# Import our modules
from ds_petri.utils.seed_manager import create_seed_instruction_file, cleanup_seed_files
from ds_petri.utils.orchestrator import run_sequential_evals, run_parallel_evals, extract_error_message
from ds_petri.utils.validation import validate_all_inputs, ValidationError
from ds_petri.utils.logging_config import setup_logging, log_eval_start, log_eval_complete
from ds_petri.utils.checkpoint import (
    create_checkpoint,
    save_checkpoint,
    load_checkpoint,
...
from ds_petri.parsers.score_parser import collect_transcripts, get_all_dimension_names
from ds_petri.utils.results_presenter import display_results_table, export_json_results
from ds_petri.utils.callback import invoke_callback
from ds_petri.utils.config_manager import (
...
def create_example(
    ...
    Example:
        ds-petri create-example --output my_config.yaml
        ds-petri create-example --output my_config.json --format json
    """
    try:
...
def show_config(
    ...
    Example:
        ds-petri show-config my_config.yaml
    """
    try:
...
def save_config(
    ...
    provider: str = typer.Option(
        "chutes",
        help="Provider for model API (chutes or openrouter)",
    ),
...
def run(
    ...
    run_id: Optional[str] = typer.Option(
        None,
        help="Custom run identifier for this evaluation (auto-generated if not provided)",
    ),
):
    """
    ...
    Example:
        ...
        ds-petri run --config my_eval.yaml

        # Load from config, override with CLI args
        ds-petri run --config my_eval.yaml --seed "New seed instruction"

        # Run in parallel with custom worker count
        ds-petri run --config my_eval.yaml --parallel --max-workers 9
    """
    try:
        print("=" * 80)
        ...
        cli_config = EvalConfig(
            ...
            provider=provider or file_config.provider,
            ...
            run_id=run_id,  # Use CLI run_id if provided, else will use file_config.run_id or generate new one
        )
        ...
        # Format models with provider prefix
        formatted_models = [format_model_with_provider(model, provider) for model in model_list]
        formatted_auditor = format_model_with_provider(validated_auditor, provider)
        formatted_judge = format_model_with_provider(validated_judge, provider)
        
        logger.info(f"Formatted models with provider '{provider}'")
        for original, formatted in zip(model_list, formatted_models):
            if original != formatted:
                logger.debug(f"  {original} -> {formatted}")

        # Log evaluation start
        log_eval_start(logger, formatted_models, validated_seed)
        ...
        checkpoint = create_checkpoint(
            ...
            failed_models={},
            seed_file=seed_file,
            auditor=formatted_auditor,
            judge=formatted_judge,
        ...
        print(f"\n[*] Running {len(formatted_models)} Petri evaluations ({execution_mode})...")
        start_time = time.time()

        if parallel:
            eval_results = run_parallel_evals(
                seed_instruction_file=seed_file,
                target_models=formatted_models,
                auditor_model=formatted_auditor,
                judge_model=formatted_judge,
                max_turns=validated_max_turns,
                output_dir=validated_output_dir,
                logger=logger,
        ...
        transcripts, parsing_errors = collect_transcripts(validated_output_dir, run_identifiers=formatted_models, run_id=run_id)
